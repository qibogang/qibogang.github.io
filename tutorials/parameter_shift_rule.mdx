...
$$ \partial\_\{\mu\} = \frac\{1\}\{r\} \bigl( f(\mu^+) - f(\mu^-) \bigr), $$

where $\mu^\{\pm\} = \mu \pm s$ and $s = \pi / 4r$. Finally, if we pick $G$ from 
the rotations generators we can use $s=\pi/2$ and $r=1/2$.
